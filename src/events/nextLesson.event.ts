...
import {getNextLesson, getWaitSecondsData} from '@/src/events/utils.queryData'
...
const event: ClientEvent<'callback_query'> = {
  ...
  execute: async (context, storage) => {
    ...
    if (lesson === null) {
      const lessonData = getNextLesson(context.callbackQuery)
      ...
      await context.editMessageText(
        ...
        {reply_markup: INLINE_KEYBOARDS.startQuiz(section.id, section.parentSectionID)}
      )
      return
    }
