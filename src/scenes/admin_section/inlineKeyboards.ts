...
export function getSectionInlineKeyboard(section: FullSectionStorage): Markup.Markup<InlineKeyboardMarkup> {
  ...
  const subsectionsCallbackData = 'subsections'
  const subsectionsText = locales.scenes.admin_section.subsections
  const subsectionsButton = {text: subsectionsText, callback_data: subsectionsCallbackData}
  buttons.push([subsectionsButton])
